...
구현하면서 **선택지가 있었고 하나를 골랐다면** 기록합니다. 코드만 봐서는 왜 그렇게
했는지 알 수 없는 것이 대상입니다.
...
- **문제**: 만료 토큰에 500이 나가 클라이언트가 재로그인 분기를 타지 못했다
- **선택**: 만료를 인증 실패로 보고 `401` + `code: TOKEN_EXPIRED` 반환
- **대안**:
  - `419` 커스텀 코드 — 표준이 아니라 인터셉터·모니터링이 못 알아본다
  - `200` + 에러 바디 — 성공으로 집계돼 지표가 오염된다
- **트레이드오프**: 401을 즉시 로그아웃으로 처리하던 화면은 분기 수정이 필요하다
- ⭐ **볼트 승격 권장** — `/capture`로 `knowledge/decisions/`에 등록
```

### 볼트 승격 권장 판단 기준

아래 중 **하나라도** 해당하면 `⭐ 볼트 승격 권장` 줄을 붙입니다.

- 되돌리는 비용이 큰 결정 (스키마, 외부 계약, 인증 방식)
- 다른 기능·다른 레포에 영향을 주는 결정
- 나중에 "왜 이렇게 했지"가 나올 결정 (비직관적 선택, 트레이드오프를 감수한 선택)

**볼트 등록은 이 커맨드가 하지 않습니다.** 표시만 하고 실제 등록은 `/capture`가 맡습니다 —
크로스 레포 쓰기, `DEC-NNNN` 번호 부여, 충돌 감지는 볼트 워크플로우의 책임입니다.
...
**보고서 파일**: `docs/reports/20260803_31_로그인_시_500_에러.md`
...
**보고서 파일**: `docs/reports/20260803_112_설치_배너_추가.md`
...
#### 프론트엔드 - 프로필 페이지 수정
- **파일**: `src/main/java/.../controller/ProfileController.java`
- **변경 내용**: 이미지 업로드 버튼과 미리보기 기능 추가
- **이유**: 사용자가 업로드할 이미지를 미리 확인 가능하도록 구현

#### 백엔드 - 이미지 업로드 API 추가
- **파일**: `src/main/java/.../service/ProfileImageService.java`
- **변경 내용**: `POST /api/users/profile-image` 라우트 핸들러 추가
- **이유**: FormData로 받은 이미지를 저장소에 업로드하고 URL을 반환하도록 구현

#### 이미지 저장 로직 구현
- **파일**: `src/main/java/.../storage/ImageStorage.java`
- **변경 내용**: 이미지 파일을 `uploads/profile/` 디렉토리에 저장하는 로직 추가
- **이유**: 프로필 이미지 업로드 로직을 재사용 가능하게 분리
...
#### ProfileImageService
`@RequestPart MultipartFile`로 파일을 받아 `ImageStorage`에 위임
...
1. **docs/reports 폴더 확인**
   - `docs/reports/` 폴더가 없으면 자동 생성

2. **파일명 생성**
   - 형식: `[YYYYMMDD]_[번호]_[간단한설명].md`
...
3. **파일 저장**
   - 생성된 파일을 `docs/reports/` 폴더에 직접 저장
   - 전체 경로 예시: `docs/reports/20260803_31_로그인_시_500_에러.md`
...
- **위치**: `docs/reports/` 디렉토리
- **파일명**: `[YYYYMMDD]_[번호]_[간단한설명].md`
  - 예시: `20260803_31_로그인_시_500_에러.md`
  - 프로젝트 prefix는 포함하지 않음
- **형식**: Markdown
- **Git 추적**: `docs/reports/`는 **추적 대상**이다. 보고서도 커밋한다 — gitignore에 넣지 않는다
